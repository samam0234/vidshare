...
1. **E2E 테스트(Playwright)** — 지금까지 백엔드 79건 외에 실제 화면 흐름을 검증하는
   테스트가 없다. 기능이 더 늘기 전에 핵심 시나리오(로그인, 쇼츠 업로드, 댓글, 팔로우,
   메시지)부터
2. **서버 상태 캐싱(React Query/SWR)** — 페이지마다 중복 fetch가 쌓이고 있다
3. **배포 파이프라인(CI/CD)** — 어떤 CI 도구를 쓸지는 사용자와 먼저 상의 필요
4. **접근성·성능** — 기능이 어느 정도 안정된 지금이 전면 점검하기 좋은 시점
...
| 기능 | 설명 | 우선순위 |
|------|------|----------|
| ~~백엔드 자동화 테스트~~ | **완료 (066, 071, 074~078)** — `node:test` 83건 | — |
