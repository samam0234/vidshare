...
| 팔로잉 피드 화면 | `/api/follows/feed` 를 쓰는 UI (API만 있음) | 미착수 |
| 팔로워/팔로잉 목록 화면 | 숫자만 보이고 목록 화면이 없음 | 미착수 |
| 알림 실시간화 | 폴링 → SSE 또는 WebSocket | 미착수 |
| 실시간 메시지 | WebSocket 채팅 | 미착수 |
| 댓글 대댓글 | `comments` 에 `parent_id` | 미착수 |
| 재생목록 | 프로필 탭 실구현 | 미착수 |
| 신고·차단 | 모더레이션 기초 | 미착수 |
...
| 기능 | 설명 | 우선순위 |
|------|------|----------|
| ~~백엔드 자동화 테스트~~ | **완료 (066)** — `node:test` 32건 | — |
| 프론트 테스트 | 컴포넌트 테스트 (RTL 등) | P1 |
