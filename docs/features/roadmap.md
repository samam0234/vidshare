...
4. **실제 배포** — 가이드는 [배포 가이드](../deployment.md) 에 정리됨.
   **올리기 전에 크로스 도메인 세션 쿠키(`sameSite`)와 `CORS_ORIGIN` 을 반드시
   먼저 고쳐야 한다** (지금 설정 그대로 올리면 로그인이 되지 않음)
5. **CI 파이프라인** — PR마다 3개 앱의 typecheck/lint/test. 배포 자체는 호스트가 자동
...
| 배포 | **가이드 작성 완료** ([deployment.md](../deployment.md)). 실제 배포·CI는 미착수 | P1 |
