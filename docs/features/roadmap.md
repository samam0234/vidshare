...
- [x] 이용약관 `/terms`, 개인정보처리방침 `/privacy`, 사업자 정보확인 `/business` (푸터, 비회원)
- [x] 다크/라이트 테마, 공통 Navbar/Footer
- [x] 통합 검색, 팔로잉 피드, 팔로워/팔로잉 목록, 재생목록
...
### 운영 (관리자 콘솔)
- [x] `console/` 관리자 전용 Next.js 앱 (포트 3200, 081~084)
- [x] 관리자 계정 (`users.role`) + 별도 세션 쿠키, `npm run create-admin`
- [x] 신고 조회·처리 / 유저 정지 / 콘텐츠 삭제 / 문의 답변 / 대시보드

### 구조
- [x] FrontServer / BackendServer / console 분리
...
**Phase A 전량 완료 (2026-08-31).** Phase B 제품 기능도 대부분 완료.
...
| 기능 | 설명 | 상태 |
|------|------|------|
| ~~통합 검색~~ | 쇼츠·롱폼·커뮤니티·유저 서버 검색 | **완료 (063)** |
| ~~알림 수신 거부 서버 반영~~ | `users.notifications_enabled` + 생성 단계 차단 | **완료 (064)** |
| ~~팔로우~~ | 팔로우/언팔, 팔로워 목록, 팔로잉 피드 API | **완료 (065)** |
| ~~팔로잉 피드 화면~~ | `/following` | **완료 (067)** |
| ~~팔로워/팔로잉 목록 화면~~ | `/profile/:id/followers`, `/following` | **완료 (069)** |
| ~~댓글 대댓글~~ | `comments.parent_id`, 1단계 스레드 | **완료 (071)** |
| ~~댓글 수정·삭제~~ | 작성자 본인만, 댓글 작성 인증 필수화(보안 수정 포함) | **완료 (074)** |
| ~~신고·차단~~ | 유저/영상/댓글/커뮤니티 신고, 유저 차단(피드 필터링) | **완료 (075)** |
| ~~재생목록~~ | 프로필 탭 실구현, 상세 페이지 (영상 추가/제거) | **완료 (076)** |
| ~~알림 실시간화~~ | 폴링 → SSE (Server-Sent Events) | **완료 (077)** |
| ~~실시간 메시지~~ | WebSocket 채팅 | **완료 (078)** |

**Phase B 전량 완료 (2026-09-01).** 다음은 Phase C로 넘어간다.

### 다음에 착수할 것 (권장 순서)

1. ~~**E2E 테스트(Playwright)**~~ — **완료 (079)**. 게스트/로그인/커뮤니티/메시지(WS) 8개 시나리오
2. ~~**서버 상태 캐싱(React Query)**~~ — **완료 (080)**. `ShortsFeed` 등 낙관적
   업데이트가 얽힌 화면은 후속 작업으로 남음
...
| ~~에러 바운더리~~ | 페이지 단위 실패 격리 | **완료 (073)** |
| ~~`mock-data.ts` 제거~~ | 잔존 코드 정리 | **완료 (072)** |
| 분석 | 조회수·체류 이벤트 | P3 |
| i18n | 한/영 | P3 |
