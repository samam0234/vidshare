...
루트 [plan.md](../plan.md) 는 “왜 VidShare인지 / 어떤 방식을 골랐는지” 계획서다.  
`docs/` 는 아키텍처·로드맵·커밋 상세용이다.
...
| 상황 | 볼 문서 |
|------|---------|
| 앱을 처음 실행한다 | 루트 `README.md` **빠른 시작** (백엔드→프론트→콘솔, 약관 페이지 확인) |
| 기획·계기·방식 비교를 본다 | 루트 `plan.md` |
